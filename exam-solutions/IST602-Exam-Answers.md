...
**Total Marks:** 75 (25 per question)
...
| Book | Title | Language | Author | Year | Price | Category |
|------|-------|----------|--------|------|-------|----------|
...
| Topic | Key Concepts |
|-------|--------------|
| **XML Well-Formed** | Single root, proper nesting, quoted attributes, escaped chars |
| **XML Valid** | Well-formed + conforms to DTD/Schema |
| **DTD vs Schema** | DTD: simpler, older; Schema: typed, namespace-aware, complex |
| **TypeScript** | Type-checking language that transpiles to JavaScript |
| **Transpiler** | Converts TypeScript → JavaScript, removes types |
| **Angular Component** | Decorator + Class + Template + Styles |
| **Selector** | CSS selector; defines component HTML tag name |
| **Data Binding** | Property (→), Event (←), Interpolation (→), Two-way (↔) |
| ***ngFor** | Structural directive for looping arrays |
| **Template Syntax** | `{{ }}` interpolation, `[prop]="expr"` binding, `(event)="fn()"` |

---

**Master's Level Key Takeaways**:
- XML provides standardized data representation and validation mechanisms
- TypeScript enhances JavaScript with static typing for large-scale development
- Angular's component-based architecture enables scalable web applications
- Understanding these technologies is essential for modern enterprise development
